        ...
        /* if (Input.mousePosition.x < Screen.width / 2)
         {
             ...
             // If the captured image's width and/or height is greater than 1024px, downscale it
             TakePicture(1024);
         }*/
        /*else
        {
    ...
    private void TakePicture(int maxSize)
    {
        NativeCamera.Permission permission = NativeCamera.TakePicture((path) =>
        {
            ...
            if (path != null)
            {
                ...
                Texture2D texture = NativeCamera.LoadImageAtPath(path, maxSize: 1024, false);
                if (texture == null)
                {
                    Debug.LogError("Texture is null, image loading failed.");
                    return;
                }
                else
                {
                    Debug.Log("Texture successfully loaded: " + texture.width + "x" + texture.height);
                }
               
                UIManager.instance.GetScreen<LeagueChatScreen>().UploadImage(texture);
